...
## [1.31.1.0] - 2026-05-10 — Oracle fork sync

Merged upstream v1.16.0.0..v1.31.0.0 — a big sync. Highlights: AskUserQuestion fallback delete + harness primitives (v1.31.0.0), 21-community-PR fix wave + Windows CI extension (v1.30.0.0), worktree-aware gbrain code sources (v1.29.0.0), browse `--headed/--proxy/--navigate` + webdriver-only stealth (v1.28.0.0), `/setup-gbrain` Path 4 remote MCP + brain → artifacts rename (v1.27.0.0), `/sync-gbrain` skill + native code-surface orchestrator (v1.26.3.0), GSTACK REVIEW REPORT delete-then-append (v1.26.4.0), plan-eng-review STOP gate + report-at-bottom contract (v1.26.2.0). Oracle remains intact on this fork.
