...
## [1.44.1.0] - 2026-05-25 — Oracle fork sync

Merged upstream v1.32.0.0..v1.44.0.0 — a big sync. Highlights: long-lived sidebar with keepalive + transparent re-attach + scrollback replay (v1.44.0.0), browse launch hardening + headed-mode idle-timer fix (v1.43.x), iOS device-farm with 5 skills + Mac daemon + Tailscale (v1.43.0.0), Daegu wave with 23 community-filed bugs + PTY classifier enforcement (v1.42.0.0), 7 HIGH bugs from external audit (v1.41.1.0), gbrain sync hardening (v1.40.0.0). Oracle remains intact on this fork.
