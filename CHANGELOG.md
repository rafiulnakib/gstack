...
## [1.55.1.0] - 2026-06-01 — Oracle fork sync

Merged upstream v1.45.0.0..v1.55.0.0. Highlights: gbrain data-loss guards + browser crash-loop fix (v1.55.0.0), `/ship` carved into skeleton + on-demand sections (-59% always-loaded) (v1.54.0.0), smarter PII/secrets/legal redaction across /spec, /ship, /cso, /document-* (v1.53.0.0), non-interactive plan-tune hook install (v1.53.1.0), brain-aware planning across 5 skills (v1.52.1.0), make-pdf emoji rendering on Linux (v1.52.2.0), plan-tune explicit consent + first-run wizard (v1.52.0.0), $B memory diagnostic + 4 CDP-resource leak fixes (v1.51.0.0), AskUserQuestion split rule + runtime AUTO_DECIDE carve-out (v1.48.0.0). Oracle remains intact on this fork.
